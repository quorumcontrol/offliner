
import { WrappedBlockService, Repo, Tupelo, tomlToNotaryGroup } from 'tupelo-wasm-sdk';
import { importLicense } from '..';
import CID from 'cids';
const IpfsBlockService: any = require('ipfs-block-service');

const notaryGroup = tomlToNotaryGroup(`
id = "wasmdocker"
BootstrapAddresses = [
    "/ip4/172.16.246.10/tcp/34001/ipfs/16Uiu2HAm3TGSEKEjagcCojSJeaT5rypaeJMKejijvYSnAjviWwV5",
    "/ip4/127.0.0.1/tcp/34001/ipfs/16Uiu2HAm3TGSEKEjagcCojSJeaT5rypaeJMKejijvYSnAjviWwV5",
    "/ip4/127.0.0.1/tcp/50000/ws/ipfs/16Uiu2HAm4hjC7zZTSN5KAp187vn52bZMsC8VaVdtrci7HD4aZcU7",
    "/ip4/172.16.246.11/tcp/50000/ws/ipfs/16Uiu2HAm4hjC7zZTSN5KAp187vn52bZMsC8VaVdtrci7HD4aZcU7",
]
[[signers]]
VerKeyHex = ""
DestKeyHex = "0x04f6dee3f7da1da58afd6ee58ea6b858fb67664fc6e2240bb6e3a75c0e1db9bbef5f413c8604bb864513d3cf27eca60b539b048b2a08f8799570c14dfb73f3f391"

[[signers]]
VerKeyHex = ""
DestKeyHex = "0x0438b196bddb9c3ec395b8ccb07bdab44ec768c084e7141b09ac5638d47fffbd5e7b7623f499a2e714e31464a356a0e30ad7c93045b6cd9957b45e957cc15dcb99"

[[signers]]
VerKeyHex = ""
DestKeyHex = "0x0468924bd1341b5cec1fed888aaf1e3caa94e7d0f13d4f4573b01b296374b9e710a58a7b40e7161c0bcf7fd41832441ca21076f3846e854c8d8c640f2469a552b1"
`)


const getRepo = async () => {
    const repo = new Repo(".tmp/license")

    try {
        console.log('init')
        await repo.init({});
        console.log('open')
        await repo.open();
    } catch(e) {
        console.error('rejecting: ', e)
        throw e
    }
   
    console.log('return')
    return repo;
};

const main = () => {
    return new Promise(async (resolve, reject) => {
        console.log("main()")
        try {
            console.log("repo()")

            const repo = await getRepo()
            console.log("tip()")

            let currentTipBits = new Uint8Array()
            let currentTip
            try {
                currentTipBits = await repo.get("_current")
                console.log("current tip bits: ", currentTipBits)
            } catch(e) {
                // do nothing for now, it's not found
            }

            if (currentTipBits && currentTipBits.byteLength > 0) {
                currentTip = new CID(Buffer.from(currentTipBits))
            }
            console.log("service()")

            let bs = new IpfsBlockService(repo.repo)
            const service = new WrappedBlockService(bs)
            console.log("import()")

            const [tree, state] = await importLicense(service, "license.cbor")

            if (CID.isCID(currentTip) &&  tree.tip.equals(currentTip as CID)) {
                console.log("local tree is identicial to import")
                return
            }

            console.log("verifiying current state")
            const verified = await Tupelo.verifyCurrentState(notaryGroup, state)
            if (verified) {
                console.log("updating local tree to new tip: ", tree.tip.toBaseEncodedString())
                await repo.put("_current", tree.tip.buffer)
                resolve(true)
                return
            }
        } catch (e) {
            console.error('rejecting: ', e)
            reject(e)
        }
        reject(false)
    })
}

main().then(() => {
    console.log("updated")
    process.exit(0);
}, () => {
    console.error("unverified tree")
    process.exit(1);
})
